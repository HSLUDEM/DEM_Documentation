+----------------+------+-----------+------------------------------------------+
| Attribute      | Unit | Data type | Description                              |
+================+======+===========+==========================================+
| year           | —    | int       | Year to be simulated. Takes into account |
|                |      |           | future weather scenario and retrofitting |
|                |      |           | of buildings. Options: '2023', '2030',   |
|                |      |           | '2040', '2050'.                          |
+----------------+------+-----------+------------------------------------------+
| rcp_scenario   | —    | str       | Representative Concentration Pathways    |
|                |      |           | (RCP) for future climate. Options:       |
|                |      |           | 'RCP26'.                                 |
+----------------+------+-----------+------------------------------------------+
| ev_integration | —    | bool      | If set to 'true', electrification of     |
|                |      |           | mobility sector will be considered and   |
|                |      |           | electricity demand profiles will be      |
|                |      |           | adjusted accordingly.                    |
+----------------+------+-----------+------------------------------------------+
| ev_flexibility | —    | bool      | If set to 'true', the available          |
|                |      |           | flexibility in demand shifting stemming  |
|                |      |           | from the EV demand will be considered in |
|                |      |           | the optimisation.                        |
+----------------+------+-----------+------------------------------------------+
